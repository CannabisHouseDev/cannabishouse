...
6.times do |i|
  ...
  user.profile.credits = 10000000 if i == 1
  user.profile.save
...
3.times do |i|
  MaterialType.create(name: ["Sativa", "Indica", "Hybrid"][i])
end
...
Material.all.each do |material|
  ...
  if transfer[0]
    ...
    puts "Could not transfer #{material.name}, state: #{material.aasm_state}"
  end
end
